...
# with pynannote overlap
overlap=0

pyannote_pretrained_model=None
################################################################################
# Logging
################################################################################
...
if [ $stage -le 0 ]; then
  ...
  if [ $overlap == 1 ]; then
    filenums=`awk '{print $1}' $data_dir/wav.scp | wc -l`
    
    mkdir -p $output_dir/per_file_overlap
    if [ ! `ls $output_dir/per_file_overlap | wc -l`  -eq $filenums ]; then
      echo "overlap detection"
      $cmd JOB=1:$nj $output_dir/log/predict_overlap.JOB.log \
      $PYTHON local/diarization/predict_overlap.py \
          --pyannote_pretrained_model $pyannote_pretrained_model \
          $sdata/JOB $output_dir/
    fi
   
    $cmd JOB=1:$nj $output_dir/log/VB_resegmentation_overlap.JOB.log \
      $PYTHON local/diarization/VB_resegmentation_overlap.py \
        $args \
